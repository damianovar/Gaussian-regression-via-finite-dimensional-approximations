...
%
%
function GenerateMonodimensionalStableSplineKernel( tGenerator, fExponentialDecay )
	%
	%
	% for readability
	...
	for iFirstInputLocationIndex = 1:iNumberOfInputLocations;
		...
		for iSecondInputLocationIndex = 1:iNumberOfInputLocations;
			...
			tGenerator.ffKernel(	iFirstInputLocationIndex,		...
				...
				( exp(- fExponentialDecay * fMax)^2 )				...
				/													...
				(													...
						exp(- fExponentialDecay * fMin)				...
					-	(exp(- fExponentialDecay * fMax) / 3)		...
				);
			%
		end;%
		%
